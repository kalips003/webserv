...
int Ft_Post::ft_do() {
	printLog(METHOD, "POST method called", 1);

	// const HttpRequest& req = getRequest();

	// int	size = req.getBodySize();
	// std::string	path = req.getPath();

	// std::cout << "This is the size : " << size << "\n" << "This is the path : " << path << "\n";


	// std::string	FileName = "where_to_go";
...
// add path to root
	...
	if (rtrn)
		return rtrn;
	oss msg; msg << "Full path where to post the asked ressource: " << ressource;
	printLog(TEST, msg.str(), 1);

	std::string FileName = ressource + "where_to_go";
	std::ofstream ofs(FileName.c_str(), std::ios_base::binary);
	ofs << req.getBody();
