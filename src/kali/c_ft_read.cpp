...
enum ConnectionStatus connection::ft_read(char *buff, size_t sizeofbuff) {
    ...
    std::cerr << C_134 "msg received (" RESET << bytes << C_134 " bytes): \n[" RESET << str_buff << C_134 "]" RESET << std::endl;

    if (_status == FIRST)
        _status = parse_header_first_rec(_buffer + str_buff);

    if (_status <= READING_HEADER)
        _status = check_buffer_for_rnrn(buff);
    else if (_status == READING_BODY) {

        if (_request.body_size < 4096)
            _request.body += str_buff;
        else if (_request.fd_body >= 0)
            write(_request.fd_body, buff, bytes); // write(_request.fd_body, buff, bytes)
        _request.body_bytes_received += bytes;
        if (_request.body_bytes_received >= static_cast<size_t>(_request.body_size))
            return DOING;
...
///////////////////////////////////////////////////////////////////////////////]
///////////////////////////////////////////////////////////////////////////////]
enum ConnectionStatus connection::parse_header_first_rec(std::string first_rec) {
...
enum ConnectionStatus    connection::check_buffer_for_rnrn(char *buff) {          
    ...
    while (*buff != '\0') {
        ...
        if (*buff == delim[_request.header_delim_progress]) {
            _request.header_delim_progress++;

            if (_request.header_delim_progress == size_delim) {
            ...
            _request.header_delim_progress = (*buff == delim[0]);
...
enum ConnectionStatus    connection::parse_header_wrapper(char *buf) {
    ...
    _request.body_size = _request.isThereBody();
    if (_request.body_size < 0) {
        printErr(ERR9 "bad body-size");
        return create_error(400);
    }

    _request.body = std::string(buf);
    _request.body_bytes_received = _request.body.size();
    // _buffer = "";
    _buffer.clear();
    
    _body_task = createTask(_request.method, _request, _answer);
    if (_body_task->_status) {
        std::cerr << "BodyTask status: " << _body_task->_status << std::endl;
        // return create_error(_body_task->_status);
    }
    if (!_request.body_size || _request.body_bytes_received >= static_cast<size_t>(_request.body_size))
        return DOING;
    else 
        return READING_BODY;
}
